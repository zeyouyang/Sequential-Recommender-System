...
#Position Encoding(要選最大序列長度)
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, maxlen=99):
...
class SequenceModel(nn.Module):
    def __init__(self,opt, num_games, num_durations, num_intervals, game_embed_size, duration_embed_size, interval_embed_size, hidden_size, num_heads,num_layers):
        ...
        self.dropout=0.05
        self.d_model=self.hidden_size*3
        ...
        self.transformer = nn.Transformer(d_model=self.d_model, nhead=self.num_heads, num_encoder_layers= self.num_layers,dim_feedforward=self.dim_feedforward)
        ...
        # Output layers
        self.out_game = nn.Linear(hidden_size*3, num_games)
        self.out_duration = nn.Linear(hidden_size*3, num_durations)
        self.out_interval = nn.Linear(hidden_size*3, num_intervals)
    ...
    def forward(self, games, durations, intervals, A_game, A_duration, A_interval,alias_inputs_game,alias_inputs_duration,alias_inputs_interval):
        ...
        sequence_embed = torch.cat((seq_hidden_game,seq_hidden_duration,seq_hidden_interval),dim=2) #32*99*(128*3)
        ...
        sequence_embed_pe = sequence_embed+pe

        sequence_embed_t = sequence_embed_pe.permute(1, 0, 2)
        
        sequence_embed_output = self.transformer(sequence_embed_t,sequence_embed_t)
        sequence_embed_output = sequence_embed_output.permute(1, 0, 2)
  
        return sequence_embed_output
...
def train_test(model, train_data, test_data):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        loss = 0.4*loss_game + 0.3*loss_duration + 0.3*loss_interval
        loss.backward()
