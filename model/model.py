...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, maxlen=69):
...
class SequenceModel(nn.Module):
    ...
    def forward(self, games, durations, intervals, A_game, A_duration, A_interval,alias_inputs_game,alias_inputs_duration,alias_inputs_interval):
        ...
        sequence_embed = torch.cat((seq_hidden_game,seq_hidden_duration,seq_hidden_interval),dim=2) #8*99*(256*3)
        ...
        sequence_embed_pe=self.dropout1(sequence_embed_pe)
...
def train_test(model, train_data, test_data):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        loss = 0.6*loss_game + 0.2*loss_duration + 0.2*loss_interval
        loss.backward()
